...
\textbf{【解析】}由题意可知, \(a = 1\) 或 \({a}^{2} = 1\) ,即 \(a = 1\) 或 \(a =  - 1\) ,
...
\textbf{知识点}：集合运算、复数\
\textbf{难度}：$\star$ 基础题\
\textbf{评价}：本题为基础题，考查集合运算、复数的基本概念和应用。
\end{analysisbox}

2. 解不等式 \(\frac{1}{x} > 1\) ,则不等式的解集是___

\textbf{【答案】} \(\left( {0,1}\right)\)

\textbf{【解析】}根据题意,由 \(\frac{1}{x} > 1\) ,得 \(\frac{1 - x}{x} > 0\) ,即 \(x\left( {1 - x}\right)  > 0\) ,解得 \(0 < x < 1\) ,故解集为 \(\left( {0,1}\right)\) .

\begin{analysisbox}[考点分析]
\textbf{知识点}：不等式\
\textbf{难度}：$\star$ 基础题\
\textbf{评价}：本题为基础题，考查不等式的基本概念和应用。
\end{analysisbox}

3. 已知角 \(\alpha\) 的终边经过点 \(P\left( {1, - 2}\right)\) ，则 \(\sin \alpha  =\) ___.

\textbf{【答案】} \(- \frac{2\sqrt{5}}{5}\)
...
\textbf{【解析】}设等差数列 \(\left\{  {a}_{n}\right\}\) 的公差为 \(d\) ,依题意, \(d = \frac{{a}_{26} - {a}_{6}}{{26} - 6} = \frac{{11} - 1}{20} = \frac{1}{2}\) ,
...
\textbf{知识点}：复数、数列\
\textbf{难度}：$\star$ 基础题\
\textbf{评价}：本题为基础题，考查复数、数列的基本概念和应用。
\end{analysisbox}

5. 双曲线 \(\frac{{x}^{2}}{9} - \frac{{y}^{2}}{16} = 1\) 的渐近线方程是___.

\textbf{【答案】} \(y =  \pm  \frac{4}{3}x\)

\textbf{【解析】}因为双曲线方程为 \(\frac{{x}^{2}}{9} - \frac{{y}^{2}}{16} = 1\) , 所以双曲线的渐近线方程为 \(\frac{{x}^{2}}{9} - \frac{{y}^{2}}{16} = 0\) ,即 \(y =  \pm  \frac{4}{3}x\) ,

\begin{analysisbox}[考点分析]
\textbf{知识点}：圆锥曲线\
\textbf{难度}：$\star$ 基础题\
\textbf{评价}：本题为基础题，考查圆锥曲线的基本概念和应用。
\end{analysisbox}
...
\textbf{【解析】}第一步先从非零的四个数中选择一个作为百位数字,有 4 种选法,
...
\textbf{【解析】}\(\because y = {\sin }^{4}x + {\cos }^{4}x = {\left( {\sin }^{2}x + {\cos }^{2}x\right) }^{2} - 2{\sin }^{2}x \cdot  {\cos }^{2}x\)
...
\textbf{知识点}：三角函数\
\textbf{难度}：$\star$ 基础题\
\textbf{评价}：本题为基础题，考查三角函数的基本概念和应用。
\end{analysisbox}

8. 已知函数 \(y = {x}^{3} + {x}^{2} + {ax}\) 有三个不同的零点,则实数 \(a\) 的取值范围是___

\textbf{【答案】} \(\left( {-\infty ,0}\right)  \cup  \left( {0,\frac{1}{4}}\right)\)

\textbf{【解析】}由 \({x}^{3} + {x}^{2} + {ax} = 0\) ,得 \(x = 0\) 或 \({x}^{2} + x + a = 0\) ,
...
\textbf{知识点}：综合知识点\
\textbf{难度}：$\star$ 基础题\
\textbf{评价}：本题为基础题，考查综合知识点的基本概念和应用。
\end{analysisbox}
...
\textbf{【答案】} \(\left\lbrack  {\frac{1}{2},1}\right\rbrack\)

\textbf{【解析】}\(\overrightarrow{a}\) 在 \(\overrightarrow{b}\) 方向上的数量投影为 \(\frac{\overrightarrow{a} \cdot  \overrightarrow{b}}{\left| \overrightarrow{b}\right| } = \frac{3\cos x + \sqrt{3}\sin x}{2\sqrt{3}} = \frac{2\sqrt{3}\cos \left( {x - \frac{\pi }{6}}\right) }{2\sqrt{3}} = \cos \left( {x - \frac{\pi }{6}}\right)\) , \(x \in  \left\lbrack  {0,\frac{\pi }{2}}\right\rbrack  ,\;x - \frac{\pi }{6} \in  \left\lbrack  {-\frac{\pi }{6},\frac{\pi }{3}}\right\rbrack  ,\;\cos \left( {x - \frac{\pi }{6}}\right)  \in  \left\lbrack  {\frac{1}{2},1}\right\rbrack  .\)

\begin{analysisbox}[考点分析]
\textbf{知识点}：复数、平面向量/空间向量\
\textbf{难度}：$\star$ 基础题\
\textbf{评价}：本题为基础题，考查复数、平面向量/空间向量的基本概念和应用。
\end{analysisbox}
...
\textbf{【解析】}由题意,甲抛掷一次获胜的概率为 \(\frac{1}{4}\) ,失败的概率为 \(\frac{3}{4}\) ,
...
\textbf{评价}：本题为中档题，考查概率统计的基本概念和应用。
\end{analysisbox}

\begin{tipbox}[教学提示]
本题是试卷中的重点题目，建议讲解时：
egin{enumerate}
\item 引导学生仔细审题，提取关键信息
\item 强调相关概念和公式的正确应用
\item 注意常见错误和易错点的提醒
\item 可适当拓展相关题型和解题方法
\end{enumerate}
...
\textbf{【答案】} \(\arctan \frac{1}{2}\)

\textbf{【解析】}由函数 \(y = {x}^{2}, x \in  \left\lbrack  {0,1}\right\rbrack\) ,知 \({y}^{\prime } = {2x}, x \in  \left\lbrack  {0,1}\right\rbrack\) .
...
\textbf{【解析】}圆柱体中,最远两点间的距离为 \(l\left( V\right)  = \sqrt{{\left( 2R\right) }^{2} + {h}^{2}}\) ,
...
\textbf{知识点}：立体几何\
\textbf{难度}：$\star\star$ 中档题\
\textbf{评价}：本题为中档题，考查立体几何的基本概念和应用。
\end{analysisbox}
...
\textbf{【解析】}因为 \(\alpha\) 是 \(\beta\) 的充分非必要条件,所以 \(\alpha\) 成立时 \(\beta\) 一定成立
...
\textbf{知识点}：集合运算、复数\
\textbf{难度}：$\star\star$ 中档题\
\textbf{评价}：本题为中档题，考查集合运算、复数的基本概念和应用。
\end{analysisbox}
...
\textbf{【解析】}由表格数据可得随着动物体重增加,脉搏率逐渐减小,即随着 \(W\) 增加, \(f\) 逐渐减小. 又函数 \(y = \ln x\) 在 \(\left( {0, + \infty }\right)\) 上单调递增,则随着 \(x = \ln W\) 增加, \(y = \ln f\) 逐渐减小, 从而 \(x\) 与 \(y\) 负相关， \(\widehat{a} < 0\) . 注意到 \(\widehat{b} = \bar{y} - \widehat{a}\bar{x}\) ,
...
\textbf{知识点}：复数、统计\
\textbf{难度}：$\star\star$ 中档题\
\textbf{评价}：本题为中档题，考查复数、统计的基本概念和应用。
\end{analysisbox}
...
\textbf{【解析】}对于 \(\mathrm{A}\) ,令 \(\lambda  = 3\) ,则 \({a}_{n + 1} = \frac{\left( {{3n} - 2}\right) {a}_{n}}{{n}^{2} + 1} \Rightarrow  \frac{{a}_{n + 1}}{{a}_{n}} = \frac{{3n} - 2}{{n}^{2} + 1}\) ,
...
\textbf{评价}：本题为难题，考查不等式、复数、数列的基本概念和应用。
\end{analysisbox}
...
\textbf{【解析】}设 \(\overrightarrow{a}\text{ 、 }\overrightarrow{b}\text{ 、 }\overrightarrow{c}\) 的坐标分别为 \(\overrightarrow{a} = \left( {{a}_{1},{a}_{2}}\right) ,\overrightarrow{b} = \left( {{b}_{1},{b}_{2}}\right) ,\overrightarrow{c} = \left( {{c}_{1},{c}_{2}}\right)\) ,
...
\begin{analysisbox}[考点分析]
\textbf{知识点}：不等式、复数、平面向量/空间向量\
\textbf{难度}：$\star\star\star$ 难题\
\textbf{评价}：本题为难题，考查不等式、复数、平面向量/空间向量的基本概念和应用。
\end{analysisbox}
...
\textbf{【答案】} (1)见解析; (2) \(\frac{\pi }{6}\)

\textbf{【解析】}(1) 由题可建立如图所示的空间直角坐标系. 设 \({AC} = a,{BC} = b,{AD} = c.\)
...
\textbf{【答案】} (1)0.0027; \(\left( 2\right) \alpha  = {0.04}\) ，判定规则在正常状态下误判率约 4.2%，虽偏低但仍存在误报，基本合理但略偏保守；(3) \(\beta  = {0.19},{5700}\) 元

\textbf{【解析】}(1) 令 \(Z = \frac{X - \mu }{\sigma } = \frac{X}{20} \sim  N\left( {0,1}\right)\) ,
...
\textbf{知识点}：复数、概率统计\
\textbf{难度}：$\star\star\star\star$ 压轴题\
\textbf{评价}：本题为压轴题，考查复数、概率统计的基本概念和应用。
\end{analysisbox}
...
\textbf{【答案】} (1)4; (2)最大值为 4，最小值为 2; (3) \(a = 8\) ，四边形 \({ONPM}\) 面积的最大值为 16

\textbf{【解析】}(1) 当点 \(P\) 在椭圆的上顶点时, \(P\left( {0,2}\right) ,{l}_{PM} : y = \frac{x}{2} + 2,{l}_{PN} : y =  - \frac{x}{2} + 2\) ,
...
\begin{analysisbox}[考点分析]
\textbf{知识点}：复数、平面向量/空间向量、圆锥曲线、圆的方程\
\textbf{难度}：$\star\star\star\star$ 压轴题\
\textbf{评价}：本题为压轴题，考查复数、平面向量/空间向量、圆锥曲线、圆的方程的基本概念和应用。
\end{analysisbox}
...
\textbf{【答案】} (1)见解析, \(\left( {0,\frac{1}{2}}\right) ;\left( 2\right) t = 1\) ; (3)见解析.

\textbf{【解析】}(1) 由 \(\sigma \left( x\right)  = \frac{1}{1 + {\mathrm{e}}^{-x}}\) ,得 \(\sigma \left( {-x}\right)  = \frac{1}{1 + {\mathrm{e}}^{x}}\) ,求和可得:
...
\textbf{知识点}：不等式、复数、数列\
\textbf{难度}：$\star\star\star\star$ 压轴题\
\textbf{评价}：本题为压轴题，考查不等式、复数、数列的基本概念和应用。
\end{analysisbox}
