...
\begin{document}
\section*{2026 届嘉定区高三下二模数学试卷}

\section*{一、填空题}

1. 已知集合 \(A = \left\{  {a,{a}^{2}}\right\}\) ，且 \(1 \in  A\) ，则 \(a =\) \_\_\_\_\_.
...
2. 解不等式 \(\frac{1}{x} > 1\) ,则不等式的解集是\_\_\_\_\_

【答案】 \(\left( {0,1}\right)\)

【解析】根据题意,由 \(\frac{1}{x} > 1\) ,得 \(\frac{1 - x}{x} > 0\) ,即 \(x\left( {1 - x}\right)  > 0\) ,解得 \(0 < x < 1\) ,故解集为 \(\left( {0,1}\right)\) .

3. 已知角 \(\alpha\) 的终边经过点 \(P\left( {1, - 2}\right)\) ，则 \(\sin \alpha  =\) \_\_\_\_\_.
...
4. 已知 \(\left\{  {a}_{n}\right\}\) 是等差数列， \({a}_{6} = 1,{a}_{26} = {11}\) ，则 \({a}_{2026} =\) \_\_\_\_\_.
...
7. 函数 \(y = {\sin }^{4}x + {\cos }^{4}x\) 的最小正周期是\_\_\_\_\_.
...
8. 已知函数 \(y = {x}^{3} + {x}^{2} + {ax}\) 有三个不同的零点,则实数 \(a\) 的取值范围是\_\_\_\_\_
...
9. 已知向量 \(\overrightarrow{a} = \left( {\cos x,\sin x}\right) ,\overrightarrow{b} = \left( {3,\sqrt{3}}\right)\) ，且 \(x \in  \left\lbrack  {0,\frac{\pi }{2}}\right\rbrack\) ，则 \(\overrightarrow{a}\) 在 \(\overrightarrow{b}\) 方向上的数量投影的取值范围为\_\_\_\_\_
...
10. 已知正四面体上的四个面上分别写有 1、2、3、4，游戏中甲、乙轮流抛掷该四面体，谁抛出底面数字等于 4 则获胜且游戏结束. 甲先开始，则甲获胜的概率为\_\_\_\_\_.
...
11. 将函数 \(y = {x}^{2},x \in  \left\lbrack  {0,1}\right\rbrack\) 的图象绕坐标原点逆时针方向旋转角 \(\theta \left( {0 \leq  \theta  \leq  \alpha }\right)\) 得到曲线 \(C\) . 若对于每一个角 \(\theta\) ，曲线 \(C\) 都是一个函数的图象，则 \(\alpha\) 的最大值为\_\_\_\_\_.

【答案】 \(\arctan \frac{1}{2}\)

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ffh8ilb0pc73deo6o0_2_1152_1256_329_294_0.jpg}
\end{center}
\hspace*{3em} 
...
12. 在包装设计中,常用长度和宽度描述物体体型. 长度 \(l\left( V\right)\) 定义为物体上最远两点间的距离,宽度 \(t\left( V\right)\) 定义为能夹住物体的两平行平面间的最小距离, 即存在一对平行平面, 使得物体上的所有点均位于两平面之间 (包括平面上). 现有一圆柱,其底面半径 \(R\) 与高 \(h\) 可任意调节,则 \(\frac{l\left( V\right) }{t\left( V\right) }\) 的最小值为\_\_\_\_\_
...
\begin{tabular}{|c|c|c|}
\hline
动物名 & 体重 \(W/\mathrm{g}\) & 脉搏率 \(f/\left( {\text{ 次 } \cdot  {\mathrm{{min}}}^{-1}}\right)\) \\
\cline{1-3}
鼠 & 25 & 670 \\
\cline{1-3}
豚鼠 & 300 & 300 \\
...
\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ffh8ilb0pc73deo6o0_6_1131_1796_363_357_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ffh8ilb0pc73deo6o0_7_1103_243_379_348_0.jpg}
\end{center}
\hspace*{3em} 
...
参考公式与数据:
 ...
 \circled{1} 若 \(X \sim  N\left( {\mu ,{\sigma }^{2}}\right)\) ,则 \(Z = \frac{X - \mu }{\sigma } \sim  N\left( {0,1}\right)\) .
...
\begin{center}
\includegraphics[max width=0.4\textwidth]{images/bo_d7ffh8ilb0pc73deo6o0_9_1013_591_510_382_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.4\textwidth]{images/bo_d7ffh8ilb0pc73deo6o0_10_1020_1259_486_238_0.jpg}
\end{center}
\hspace*{3em} 
...
而四边形 \({ONPM}\) 是平行四边形， \({S}_{□ {ONPM}} = 2{S}_{\bigtriangleup {MON}} \leq  4\) ，当 \(\left| {OM}\right|  = \left| {ON}\right|\) 时等号成立，
...
\begin{center}
\includegraphics[max width=0.4\textwidth]{images/bo_d7ffh8ilb0pc73deo6o0_11_955_389_509_243_0.jpg}
\end{center}
\hspace*{3em} 

即 \(\left| {OM}\right| \left| {ON}\right|  \leq  {20}\) ,所以 \({S}_{\bigtriangleup {MON}} = \frac{1}{2}\left| {OM}\right| \left| {ON}\right| \sin \angle {MON} \leq  8\) , 而四边形 \({ONPM}\) 是平行四边形, \({S}_{□ {ONPM}} = 2{S}_{\bigtriangleup {MON}} \leq  {16}\) , 当 \(\left| {OM}\right|  = \left| {ON}\right|\) 时等号成立,
