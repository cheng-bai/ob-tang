...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\examkaishu 虹口区高三二模·选择题}
...
{\centering\Large\bfseries 二、选择题\par}
\vspace{0.3em}
\hrule
\vspace{0.5em}
...
{\small 14. 已知 $P\left( A\right)$ 和 $P\left( B\right)$ 分别表示事件 $A$ 和事件 $B$ 发生的概率,且 $0 < P\left( B\right)  < 1$ ,则在下列各项中，“ $A$ 和 $B$ 独立” 的充分条件是( ).}
{\small A. $P\left( A\right)  = 1 - P\left( B\right)$ B. $P\left( {A \mid  B}\right)  = P\left( {A \mid  \bar{B}}\right)$}
{\small C. $P\left( {A \cup  B}\right)  = P\left( A\right)  + P\left( B\right)$ D. $P\left( {A \mid  B}\right)  = P\left( {\bar{A} \mid  B}\right)$}
\end{solutionblock}
...
{\small 15. 一定存在各项均为正数且不为常数列的无穷等差数列 $\left\{  {a}_{n}\right\}$ ,使得 ( ).}
{\small A. $\left\{  {\sin {a}_{n}}\right\}$ 为严格增数列 B. $\left\{  {\cos {a}_{n}}\right\}$ 为公差不为零的等差数}
{\small 列.}
{\small C. $\left\{  {\cos {S}_{n}}\right\}$ 为等比数列 (其中, ${S}_{n} = \mathop{\sum }\limits_{{i = 1}}^{n}{a}_{i}$ ) D. $\left\{  {\sin \frac{1}{{a}_{n}}}\right\}$ 为周期数列}
...
{\small 16. 设 $y = f\left( x\right)$ 和 $y = g\left( x\right)$ 是两个不同的函数,且定义域和值域均为 $\mathbf{R}$ ,设 $M = \left\{  {\left( {a, b}\right)  \mid  f\left( a\right) \rangle g\left( b\right) , a, b \in  \mathbf{R}}\right\}$ ,则对于以下两个结论,说法正确的是 ( )}
{\small 结论①:若当 $\left( {a, b}\right)  \in  M$ ，恒有 $\left( {-a, b}\right)  \in  M$ ，则函数 $y = f\left( x\right)$ 一定是偶函数；}
{\small 结论②:若当 $\left( {a, b}\right)  \in  M$ ，恒有 $\left( {-a, - b}\right)  \in  M$ ，则函数 $y = g\left( x\right)$ 可以不是偶函数.}
{\small A. ①和②都正确 B. ①正确，②错误 C. ①错误，②正确 D. ①和② 都错误}
{\small ## 三、解答题}
{\small <br><br><br><br><br>}
\end{solutionblock}
