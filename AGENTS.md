...
本库是主库，`latex-maki-math` 已作为 Maki 风格 LaTeX 讲义与试卷导出子系统合并进来。

合并后的维护原则：

- `ob-tang` 负责知识管理、资料索引、教学闭环和 GitHub 同步。
- `20-Product/Maki核心讲义/` 负责 Maki 风格核心讲义：`main.tex`、`maki-math.sty`、`chapters/`。
- `04-试卷汇总/` 负责真题、模考、学生版、教师版和必要图片。
- `Skills/` 负责可复用技能与导出工具。
- `00-System/Context/` 负责 AI 规则、跨 AI 记忆、复盘与工作流规范。

不要再把另一个完整资料库直接嵌入本库。像 `mini-数学资料库/` 这种大型生产库应独立维护，或只同步稳定产物和索引。
...
- 大型附件（PDF、图片）不加入版本控制
- 对于题目必要图片，可以小范围例外纳入版本控制；但扫描原件、批量图片和可再生成产物默认不纳入。
