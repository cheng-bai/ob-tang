...
**Narrative Goal**: 正式定义复数，建立复数集合的概念

**Key Content**:
- Headline: 复数的定义
- Definition: 形如 a + bi（a、b∈R）的数称为复数
- Set: 全体复数构成集合 C
- Conditions:
  - a + bi = 0 ⇔ a = 0 且 b = 0
  - a + bi = c + di ⇔ a = c 且 b = d
- Visual: 复数 z = a + bi 的结构分解

**Visual**:
- 中央：定义框（手绘边框），黄色粉笔
- 内部："z = a + bi"，大号白色粉笔
- 标注：a（绿色，实部）、b（红色，虚部系数）、i（蓝色，虚数单位）
- 下方：集合 C 的图示，包含实数 R
- 装饰：手绘大括号、箭头

**Layout**: definition-box, 定义框居中，颜色标注
