...
\begin{document}
\section*{2026 届上海松江区高三数学二模}

\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_0_0_1_1374_2337_0.jpg}
\end{center}
\hspace*{3em} 

1. 已知集合 \(A = \{ x \mid  x > 0\}\) ，集合 \(B = \{  - 1,0,1,2\}\) ，则集合 \(A \cap  B =\) \_\_\_\_\_.

\(\{ 1,2\}\)

2. 不等式 \(\left| {3 - x}\right|  < 1\) 的解集为\_\_\_\_\_.

\(- 1 < 3 - x < 1,\;x \in  \left( {2,4}\right)\)

3. 已知复数 \(z\) 满足 \(\left( {1 + i}\right)  \cdot  z = 1 - i\) (其中 \(i\) 为虚数单位),则 \(z \cdot  \bar{z} =\) \_\_\_\_\_。

\(z = \frac{1 - i}{1 + i} = \frac{\left( {1 - i}\right) \left( {1 - i}\right) }{\left( {1 + i}\right) \left( {1 - i}\right) } =  - i,\bar{z} = i\)

\(z\bar{z} =  - {i}^{2} = 1\)

4. 已知抛物线 \({y}^{2} = {2px}\left( {p > 0}\right)\) 的准线恰好平分圆 \({\left( x + 2\right) }^{2} + {\left( y + 1\right) }^{2} = 1\) 的周长,则该抛物线的焦点坐标为\_\_\_\_\_.

准线过圆心 \(\left( {-2, - 1}\right)\) ,焦点 \(\left( {2,0}\right)\)

5. 若 \(\overrightarrow{a}\text{ 、 }\overrightarrow{b}\) 都是单位向量, \(\overrightarrow{a} \cdot  \overrightarrow{b} = 0\) ,则向量 \(\overrightarrow{b}\) 与 \(\overrightarrow{a} - \overrightarrow{b}\) 的夹角大小为\_\_\_\_\_.

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_1_149_1249_195_282_0.jpg}
\end{center}
\hspace*{3em} 

\(\left\langle  {\overrightarrow{b},\overrightarrow{a} - \overrightarrow{b}}\right\rangle   = \frac{3\pi }{4}\)

6. 已知等比数列 \(\left\{  {a}_{n}\right\}\) 的前 \(n\) 项和为 \({S}_{n},{a}_{2} = 3,{a}_{5} = {81}\) . 若 \({S}_{n} = {40}\) ,则 \(n =\) \_\_\_\_\_.

\({a}_{5} = {a}_{2}{q}^{3},\;{81} = 3{q}^{3},\;q = 3\)

\({a}_{1} = \frac{{a}_{2}}{q} = \frac{3}{3} = 1,{S}_{n} = \frac{1 - {3}^{n}}{1 - 3} = {40},n = 4\)

7. 已知函数 \(y = \left\{  \begin{array}{l} {3}^{x} - a,x \geq  0 \\  b - {\left( \frac{1}{3}\right) }^{x},x < 0 \end{array}\right.\) 为奇函数,则 \(a + b\) 的值为\_\_\_\_\_.
...
9. 已知函数 \(y = {\log }_{a}\left( {x + 2}\right)  + 1\left( {a > 0}\right.\) 且 \(\left. {a \neq  1}\right)\) 的图像过定点 \(\left( {s,t}\right)\) ,正实数 \(m\text{ 、 }n\) 满足 \({nt} - {ms} = 1\) ，则 \(\frac{1}{m} + \frac{3}{n}\) 的最小值为\_\_\_\_\_.
...
10. 在四棱柱 \({ABCD} - {A}_{1}{B}_{1}{C}_{1}{D}_{1}\) 中,底面 \({ABCD}\) 为平行四边形, \(\left| \overrightarrow{A{A}_{1}}\right|  = 2\) ,异面直线 \(A{A}_{1}\) 与 \({BD}\) 的夹角为 \(\arccos \frac{2}{5},\overrightarrow{AD} \cdot  \overrightarrow{D{C}_{1}} - \overrightarrow{AB} \cdot  \overrightarrow{B{C}_{1}} = 4\) ,则 \(\left| \overrightarrow{B{D}_{1}}\right|  =\) \_\_\_\_\_。

以 \(\overrightarrow{A{A}_{1}}\text{ 、 }\overrightarrow{AB}\text{ 、 }\overrightarrow{AD}\) 为基底

\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_2_1125_1686_366_357_0.jpg}
\end{center}
\hspace*{3em} 
...
11. 如图,某公园划出一块平整的三角形草地 \({ABC}\) ,在边 \({BC}\) 上设置一个观景点 \(D\) ,点 \(D\) 到顶点 \(C\) 的距离为 2 百米, \({AD}\) 平分 \(\angle {BAC}\) ,边 \({AB}\) 和 \({AD}\) 的长度都为 3 百米. 现需要沿着三角形草地 \({ABC}\) 的边安装一圈灯带,则该灯带的长度为\_\_\_\_\_百米.

如图,由角平分线定理得 \(\frac{AB}{AC} = \frac{DB}{DC}\) ,即 \(\frac{3}{y} = \frac{x}{2}\)  \circled{1} 

\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_3_990_714_438_323_0.jpg}
\end{center}
\hspace*{3em} 
...
联立 \circled{1}  \circled{2} ，解得 \(x = \frac{3}{2},y = 4\)

\(\therefore\) 周长 \(= 3 + x + y + 2 = \frac{21}{2}\)

12. 记号 \(\max \{ a,b\}\) 表示 \(a\text{ 、 }b\) 中取较大的数,如 \(\max \{ 1,2\}  = 2\) . 在平面直角坐标系中,定义 \(d\left( {A,B}\right)  = \max \left\{  {\left| {{x}_{1} - {x}_{2}}\right| ,\left| {{y}_{1} - {y}_{2}}\right| }\right\}\) 为两点 \(A\left( {{x}_{1},{y}_{1}}\right) \text{ 、 }B\left( {{x}_{2},{y}_{2}}\right)\) 的 “切比雪夫距离”. 已知点 \(O\) 为坐标原点,点 \(C\) 在圆 \({\left( x - 1\right) }^{2} + {\left( y - 1\right) }^{2} = 1\) 上,点 \(A\) 在直线 \({3x} + {4y} - {10} = 0\) 上, 则 \(d\left( {C,A}\right)  + d\left( {C,O}\right)\) 的最小值为\_\_\_\_\_。
...
\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_3_1037_1987_438_349_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_4_1042_434_435_347_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_6_437_752_334_319_0.jpg}
\end{center}
\hspace*{3em} 

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_6_779_752_310_302_0.jpg}
\end{center}
\hspace*{3em} 

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_6_1100_752_324_312_0.jpg}
\end{center}
\hspace*{3em} 
...
对于 \circled{1} ， \({a}_{3} = 2{a}_{2}{}^{2} - 1 = 2{\left( 2{a}_{1}{}^{2} - 1\right) }^{2} - 1 = 8{a}_{1}{}^{4} - 8{a}_{1}{}^{2} + 1 \in  \left( {1,\frac{17}{8}}\right)\)

\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_7_954_266_399_257_0.jpg}
\end{center}
\hspace*{3em} 

\(\therefore {a}_{1} \in  \left( {-\frac{3\sqrt{2}}{4}, - 1}\right)  \cup  \left( {1,\frac{3\sqrt{2}}{4}}\right)\) , \circled{1} 错误

对于 \circled{2} ， \({a}_{3} =  - {a}_{1}\)

由 \circled{1} 知 \(8{a}_{1}^{4} - 8{a}_{1}^{2} + 1 =  - {a}_{1}\)

\begin{center}
\includegraphics[max width=0.4\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_7_902_539_484_304_0.jpg}
\end{center}
\hspace*{3em} 
...
\(\therefore {a}_{1}\) 有 4 个, \circled{2}  正确,选 B
...
\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_9_165_1034_408_290_0.jpg}
\end{center}
\hspace*{3em} 

\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_9_587_1030_412_282_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.5\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_10_886_962_615_358_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.4\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_11_980_355_514_332_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.4\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_12_802_1434_480_239_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_12_1092_2229_274_113_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7ci99qlb0pc73de9lc0_14_53_481_1788_1824_0.jpg}
\end{center}
\hspace*{3em} 
