...
\begin{document}
\section*{2026 届上海市金山区高三数学二模}

\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_0_0_1_1374_2337_0.jpg}
\end{center}
\hspace*{3em} 

\section*{一、填空题(本大题共有 12 题，满分 54 分，第 1\textasciitilde 6 题每题 4 分，第 7\textasciitilde 12 题每题 5 分)考生应在答题纸的 相应位置直接填写结果.}

1. 不等式 \(\left| {x - 1}\right|  < 1\) 的解集为\_\_\_\_\_ \(\left( {0,2}\right)\) \_\_\_\_\_.

\(- 1 < x - 1 < 1,x \in  \left( {0,2}\right)\)

2. 已知复数 \(z = \left( {m + 2}\right)  + \left( {m - 1}\right) \mathrm{i}\) ( \(\mathrm{i}\) 为虚数单位)为纯虚数，则实数 \(m =\) \_\_\_\_\_-2\_\_\_\_\_.

\(\left\{  {\begin{array}{l} m + 2 = 0 \\  m - 1 \neq  0 \end{array},m =  - 2}\right.\)

3. 将 \(\sqrt{a\sqrt{a}}\left( {a > 0}\right)\) 化为有理数指数幂的形式为\_\_\_\_\_ \({a}^{\frac{3}{4}}\) \_\_\_\_\_.

\({\left( {a}^{\frac{1}{2} + 1}\right) }^{\frac{1}{2}} = {a}^{\frac{3}{4}}\)

4. 已知 \(\alpha  : 1 \leq  x \leq  4,\beta  : x \leq  m\) ,若 \(\alpha\) 是 \(\beta\) 的一个充分条件,则实数 \(m\) 的取值范围为\_\_\_\_\_ \(\lbrack 4, + \infty )\) \_\_\_\_\_.

\(m \geq  4\)

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_1_441_1277_340_125_0.jpg}
\end{center}
\hspace*{3em} 

5. 已知角 \(\alpha\) 为第四象限角，且 \(\sin \alpha  =  - \frac{4}{5}\) ，则 \(\cos \alpha  =  - \frac{3}{5}\) \_\_\_\_\_.

\(\cos \alpha  = \sqrt{1 - {\sin }^{2}\alpha } = \frac{3}{5}\)

6. 已知等差数列 \(- 3, - 1,1,\cdots\) ,则该数列的第 20 项为\_\_\_\_\_35\_\_\_\_\_.

\({a}_{1} =  - 3,d =  - 1 + 3 = 2\)

\({a}_{20} = {a}_{1} + {19d} =  - 3 + {19} \times  2 = {35}\)

7. 已知随机变量 \(X\) 的分布为 \(\left( \begin{matrix} 1 & 2 & 3 \\  {0.4} & {0.2} & a \end{matrix}\right)\) ,则期望 \(E\left\lbrack  X\right\rbrack   =\) \_\_\_\_\_2\_\_\_\_\_.

\(a = 1 - {0.4} - {0.2} = {0.4}\)

\(E\left( x\right)  = 1 \times  {0.4} + 2 \times  {0.2} + 3 \times  {0.4} = 2\)

8. 若甲乙丙丁四人组成接力队参加 \(4 \times  {100}\) 米接力赛，则甲不跑中间两棒的排法共有\_\_\_\_\_12\_\_\_\_\_种.

\_\_\_\_\_ \(\frac{\text{ × }}{}\;\frac{\text{ × }}{}\) \_\_\_\_\_

9. 已知关于 \(x\) 的一元二次方程 \({x}^{2} - x + a = 0\) 有两个不相等的正根 \(m\text{ 、 }n\) ,则 \(\frac{1}{m} + \frac{9}{n}\) 的最小值为\_\_\_\_\_16\_\_\_\_\_. \(m + n = 1\left( {m,n > 0}\right)\)
...
10. 已知在 \(\bigtriangleup  {ABC}\) 中， \({AB} = 2,{AC} = 3,\angle {BAC} = {60}^{ \circ  }\) . 若点 \(O\) 为 \(\bigtriangleup  {ABC}\) 外接圆的圆心，则 \(\overrightarrow{BC} \cdot  \overrightarrow{BO} = \frac{7}{}\) \_\_\_\_\_.

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_2_1002_964_343_312_0.jpg}
\end{center}
\hspace*{3em} 
...
11. 已知 \({S}_{n}\) 是数列 \(\left\{  {a}_{n}\right\}\) 的前 \(n\) 项和,且 \({S}_{n} = {2}^{n - 1} - 1,n \geq  1\) 且 \(n \in  \mathrm{N}\) . 若 \(f\left( x\right)  = \left( {\sin x + {a}_{1}}\right) \left( {\sin x + {a}_{2}}\right) \cdots \left( {\sin x + {a}_{5}}\right)\) ,则 \({f}^{\prime }\left( \pi \right)  =\) \_\_\_\_\_64\_\_\_\_\_.
...
\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_3_847_805_285_515_0.jpg}
\end{center}
\hspace*{3em} 
...
\section*{二、选择题(本题共有 4 题，满分 18 分，13、14 每题 4 分，15、16 题每题 5 分)每题有且只有一个正确 选项, 考生应在答题纸的相应位置, 将代表正确选项的小方格涂黑.}
...
\begin{center}
\includegraphics[max width=0.4\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_4_841_788_566_491_0.jpg}
\end{center}
\hspace*{3em} 
...
17. (本题满分 14 分) 本题共有 3 个小题, 第 1 小题满分 4 分, 第 2 小题满分 5 分, 第 3 小题满分 5 分. 绝对零度 (-273.15 \textdegree C) 是一个只能逼近而不能达到的最低温度, 那么这个数据是如何测得的? 吕同学通过查询资料，知道: \circled{1} 气体温度和气体压强存在线性关系； \circled{2} 当气体压强为 \(0\left( \mathrm{{kPa}}\right)\) 时，气体温度达到绝对零度. 以下是吕同学在一次模拟实验时, 测得某种气体温度和气体压强的相关数据:
...
数据 & 1 & 2 & 3 & 4 & 5 & 6 \\
\cline{1-7}
温度 \(\left( {{}^{ \circ  }\mathrm{C}}\right)\) & 4.07 & 16.69 & 29.42 & 45.67 & 57.06 & 73.05 \\
\cline{1-7}
压强 \(\left( {\mathrm{{kPa}}}\right)\) & 103.095 & 107.734 & 112.461 & 118.469 & 122.706 & 128.758 \\
\cline{1-7}
\hline
\end{tabular}
}
...
\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_6_146_927_209_149_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_6_357_906_203_181_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_6_873_1936_252_169_0.jpg}
\end{center}
\hspace*{3em} 
...
令 \({g}^{\prime }\left( x\right)  = 0\) ,得 \({x}_{1} = \frac{1}{a},{x}_{2} = 1\)
 ...
 \circled{1}  \(\frac{1}{a} = 1,a = 1,{g}^{\prime }\left( x\right)  \geq  0,g\left( x\right)\) 严格增,与 \(y = b\) 至多 1 个交点,舍

 \circled{2}  \(\frac{1}{a} > 1,0 < a < 1\)

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_8_765_217_266_185_0.jpg}
\end{center}
\hspace*{3em} 

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_8_1180_216_269_184_0.jpg}
\end{center}
\hspace*{3em} 

\(g\left( 1\right)  = a - 1 - a + 1 = 0\)

存在 \(b < 0,y = b\) 与 \(y = g\left( x\right)\) 有个 3 交点  \circled{3}  \(\frac{1}{a} < 1,a > 1\)

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_8_755_415_344_225_0.jpg}
\end{center}
\hspace*{3em} 

\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_8_1165_415_349_225_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.5\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_8_899_1228_604_591_0.jpg}
\end{center}
\hspace*{3em} 
...
\begin{center}
\includegraphics[max width=0.2\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_10_899_440_338_330_0.jpg}
\end{center}
\hspace*{3em} 
...
\[
\text{  \circled{1}  }b + 1 \geq   - 1 - a,\;a + b \geq   - 2
\]
...
\begin{center}
\includegraphics[max width=0.3\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_10_926_1703_419_351_0.jpg}
\end{center}
\hspace*{3em} 
...
\hspace*{1em} \(\therefore h\left( a\right)  \geq  a,h\left( b\right)  \leq  b\)
...
\hspace*{1em} \(\therefore \left| {{x}_{n} - c}\right|  = L\left| {{x}_{n - 1} - c}\right|  = {L}^{n - 1}\left| {{x}_{1} - c}\right|\)

\hspace*{2em} \(\mathop{\lim }\limits_{{n \rightarrow   + \infty }}{L}^{n - 1} = 0,\;L \in  \lbrack 0,1)\)

\hspace*{1em} \(\therefore \mathop{\lim }\limits_{{n \rightarrow   + \infty }}\left| {{x}_{n} - c}\right|  = 0\)

\hspace*{1em} \(\therefore \mathop{\lim }\limits_{{n \rightarrow   + \infty }}{x}_{n} = c\)
...
\begin{center}
\includegraphics[max width=1.0\textwidth]{images/bo_d7ci9qalb0pc73de9lfg_12_53_481_1788_1824_0.jpg}
\end{center}
\hspace*{3em} 
