---
title: "The complete claude code tutorial 完整的 claude 代码教程"
...
tags:
  - "clippings"
---
# The complete claude code tutorial 完整的 claude 代码教程
...
Now for some of you, this is easier said than done. You might not have years of software engineering experience that would allow you to think about this on your own. To this extent I have two pieces of advice: 1. Start learning. You are handicapping yourself if you never pick up on this, even if just a little bit at a time. 
...
[CLAUDE.md](https://claude.md/) is a markdown file. Markdown is a text format that AI models process extremely well, and Claude in particular handles it better than most other models I've tested.CLAUDE.md 是一个 Markdown 文件。Markdown 是一种文本格式，AI 模型处理得非常好，尤其是 Claude 比我测试过的其他大多数模型处理得更好。

When you start a Claude Code session, the first thing Claude does is read your [CLAUDE.md](https://claude.md/) file. Every instruction in that file shapes how Claude approaches your project. It's essentially onboarding material that Claude reads before every single conversation. 
...
Update it constantly. Press the # key while you're working and Claude will add instructions to your [CLAUDE.md](https://claude.md/) automatically. Every time you find yourself correcting Claude on the same thing twice, that's a signal it should be in the file. Over time your [CLAUDE.md](https://claude.md/) becomes a living document of how your codebase actually works. 

Bad [CLAUDE.md](https://claude.md/) looks like documentation written for a new hire. Good [CLAUDE.md](https://claude.md/) looks like notes you'd leave yourself if you knew you'd have amnesia tomorrow. 
...
Use external memory. If you're working on something complex, have Claude write plans and progress to actual files (I use [SCRATCHPAD.md](https://scratchpad.md/) or [plan.md](https://plan.md/)). These persist across sessions. When you come back tomorrow, Claude can read the file and pick up where you left off instead of starting from zero. Sidenote: if you have a hierarchy system of files, keeping these at thievery top is how you can get these to work for every task / feature that you decide to build out. 
...
Know when to clear. If a conversation has gone off the rails or accumulated a bunch of irrelevant context, just /clear and start fresh. It's better than trying to work through confusion. Claude will still have your [CLAUDE.md](https://claude.md/), so you're not losing your project context. Nine times out of ten, using clear is actually better than not using it, as counterintuitive as that sounds. 
...
The flywheel: Claude makes a mistake, you review the logs, you improve the [CLAUDE.md](https://claude.md/) or tooling, Claude gets better next time. This compounds. Right now I'm in the process of being able to have Claude already improve its own [Claude.md](https://claude.md/) files though. After months of iteration, systems built this way are meaningfully better than they were at launch - same models, just better configured. 
...
[CLAUDE.md](https://claude.md/) is your leverage point. Keep it short, specific, tell it why, and update constantly. This single file affects every interaction. 
...
Modern day technology is absurdly capable. If you want more tips on how to get the most out of AI for yourself or your business, subscribe to my free weekly AI newsletter: [https://varickagents.com/newsletter](https://varickagents.com/newsletter)