---
title: "给我的 AI Agent 升级装上了字节的记忆架构: OpenViking 实战分享"
source: "https://x.com/xxx111god/status/2023838143045136557"
author:
  - "[[Unknown]]"
...
但他们的系统更**体系化** — 有完整的 L0/L1/L2 分层架构，有 .abstract 索引机制，有自动化的 compounding 流程。
...
- [MEMORY.md](https://memory.md/) 存核心信息（偏好、项目、工具配置）
...
每条记忆都打标签，[memory-janitor.py](https://memory-janitor.py/) 每天凌晨自动扫描，过期的 P1/P2 移到 archive/。

**解决的问题：** 记忆不会无限膨胀，过期的自动归档。

**没解决的问题：** 即使没过期，[MEMORY.md](https://memory.md/) 也越来越大，每次全读太浪费。Bot 每次启动都要读 [MEMORY.md](https://memory.md/) 全文，这文件已经 8000+ tokens 了。

大部分时候其实用不到那么多 — 比如我问天气交易bot的结算结果，它不需要知道我的报税项目进度。
...
字节这个框架的核心不是新概念，是**把一个我们已经在讨论的模糊想法具体化了**：
...
类比一下：你不会打开电脑就把所有文件内容读一遍吧？先看文件夹结构，再打开需要的文件。
...
![图像](https://pbs.twimg.com/media/HBYd3bBWYAAdojR?format=jpg&name=large)

**新加的两个脚本：**

- [memory-abstract-gen.py](https://memory-abstract-gen.py/) — 扫描目录，自动生成/更新 .abstract
- [pre-compact-extractor.py](https://pre-compact-extractor.py/) — Context 压缩前把重要内容抢救到 [SESSION-STATE.md](https://session-state.md/)
...
这时候我意识到，P0/P1/P2 和 L0/L1/L2 这两套系统是**互补**的：

![图像](https://pbs.twimg.com/media/HBYV6W8aoAACusH?format=jpg&name=large)

**检索流程现在是：**

![图像](https://pbs.twimg.com/media/HBYWpnpWUAAPAan?format=jpg&name=large)
...
└── archive/               # 过期 P1/P🆕 = 这次从 OpenViking 学来后新加的
```
...
![图像](https://pbs.twimg.com/media/HBYX248b0AAYbHv?format=jpg&name=large)
